...
public class ParkingActionTest {
    ...
    public static void setUp() {
        ...
        observer = event -> {
            ...
            ParkingPermit permit = office.getParkingPermit(receivedEvent.getParkingPermitId());
            charges = receivedEvent.getParkingLot()
                    .getParkingCharges(permit,
                            receivedEvent.getIn(),
                            receivedEvent.getParkingLot()
                                    .getDuration(permit, receivedEvent.getIn()));
    ...
    public final void testObserverFramework() {
        ...
        ParkingEvent event = new ParkingEvent(lot,
                LocalDateTime.parse(times[0][0]),
                LocalDateTime.parse(times[0][1]),
                permit.getId(), true);
        action.parkingNotification(event);
        System.out.println("Fired event");
        assertTrue(event.getIn().compareTo(receivedEvent.getIn()) == 0);
        assertTrue(event.getOut().compareTo(receivedEvent.getOut()) == 0);
        assertTrue(event.getParkingLot().getId().equals(receivedEvent.getParkingLot().getId()));
        assertTrue(event.getParkingPermitId().equals(receivedEvent.getParkingPermitId()));
    }
