...
public class TransactionManager implements ParkingObserver {
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        ...
        ParkingPermit permit = office.getParkingPermit(event.getPermitId());
        if (permit != null) {
            if (event.getIsExit()) {
                // LocalDateTime exitTime, ParkingPermit permit,
                leave(event.getTimeIn(), event.getTimeOut(), permit, event.getParkingLot());
            } else {
                park(event.getTimeIn(), permit, event.getParkingLot());
            }
        } else {
            logger.severe("Permit " + event.getPermitId() + " is not registered");
        }
    }
